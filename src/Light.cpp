...
void Light::commitParameters()
{
    ...
    if (mSubtype == "point"_s) {
        float3 position = getParam<float3>(
            "position", float3(0.0f, 0.0f, 0.0f));
        float intensity = getParam<float>("intensity", 1.0f);
        float power = getParam<float>("power", 0.0f);

        // ANARI spec: if power is set, use it; otherwise use intensity.
        // Filament point light intensity is in candela (lm/sr).
        float finalIntensity = power > 0.0f
            ? power / (4.0f * static_cast<float>(M_PI))
            : intensity;
        ...
        float3 position = getParam<float3>(
            "position", float3(0.0f, 0.0f, 0.0f));
        float3 direction = getParam<float3>(
            "direction", float3(0.0f, 0.0f, -1.0f));
        float intensity = getParam<float>("intensity", 1.0f);
        float power = getParam<float>("power", 0.0f);
        float openingAngle = getParam<float>(
            "openingAngle", static_cast<float>(M_PI));
        float falloffAngle = getParam<float>("falloffAngle", 0.1f);
        ...
        float outerDeg = openingAngle * 180.0f / static_cast<float>(M_PI);
        ...
        float filamentIntensity = irradiance * static_cast<float>(M_PI);
